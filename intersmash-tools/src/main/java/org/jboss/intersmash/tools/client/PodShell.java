...
/**
 * Provides a shell into a running pod for executing commands.
 * This replaces XTF's {@code cz.xtf.core.openshift.PodShell}.
 */
@Slf4j
public class PodShell {
	...
	public PodShellOutput execute(String... commands) {
		...
		if (containerName == null) {
			openShift.pods().withName(podName).writingOutput(baosOutput).writingError(baosError)
					.usingListener(execListener)
					.exec(commands);
		} else {
			openShift.pods().withName(podName).inContainer(containerName).writingOutput(baosOutput)
		...
		try {
			new SimpleWaiter(() -> baosOutput.size() > 0 || baosError.size() > 0).timeout(TimeUnit.SECONDS, 10)
					.waitFor();
