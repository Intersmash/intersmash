...
public class CommandLineBasedKeystoreGenerator {
	...
	static {
		try {
			...
			processCall(caDir, "/bin/sh",
					"-c",
					"echo \"Q\" | openssl s_client -connect " + getOpenShiftHostAndPort() + " -showcerts 2>/dev/null > serversOpenSslResponse");
			processCall(caDir, "/bin/sh",
					"-c",
					getSplitCommandNameBasedOnOeratingSystem() + " -f serverCert -s serversOpenSslResponse '/^-----BEGIN CERTIFICATE-----$/' '{*}'");
			processCall(caDir, "/bin/sh",
					"-c",
					"find . -type f -not -name \"serverCert00\" -name \"serverCert[0-9][0-9]\" -exec openssl x509 -in {} -out {}.pem \\;");
			processCall(caDir, "/bin/sh",
					"-c",
					"find . -type f -name \"serverCert[0-9][0-9].pem\" -exec keytool -import -noprompt -keystore " + TRUSTSTORE_FILE_NAME + " -file {} -alias {} -storepass " + password + " \\;");
	...
	public static class GeneratedPaths {
		...
		public GeneratedPaths(final Path caPem, final Path truststore, final Path keystore, final Path keyPem, final Path certPem) {
			this.caPem = caPem;
